...
System::Void admin::cB_programme_TextChanged(System::Object^  sender, System::EventArgs^  e)
{
	if (cB_programme->FindStringExact(cB_programme->Text) == -1 && cB_programme->Enabled)
		cB_programme->SelectedIndex = 0;
	tB_prog->Text = cB_programme->Text;
	cur_prog= cB_programme->Text;
	if (cB_programme->Text == "") {
		edit_mode_lbl->Text = "New :";
		programm_nr->Text = "";
	}
	else {
		programm_nr->Text = MyRecordSet::GetProgramNr(cB_programme->Text);
		edit_mode_lbl->Text = "Edit :";
	}
}
...
System::Void admin::button_save_prog_Click(System::Object^  sender, System::EventArgs^  e)
{
	String^ area_id = MyRecordSet::GetAreaId(cB_staedte->Text, cB_gebiete->Text);
	if (tB_prog->Text != "")
		if (check_if_number(programm_nr->Text) && programm_nr->Text->Length == 4)
		{
			if (edit_mode_lbl->Text == "New :") {
				if (cB_programme->FindStringExact(tB_prog->Text) == -1) {
					MyRecordSet RC("SELECT * FROM Programme");
					MyInsertStatement("INSERT INTO Programme (ID,Gebiet_ID,Programm,Programm_Nr) VALUES (" + RC.get_new_id() + "," + area_id + ",'" + tB_prog->Text + "','" + programm_nr->Text + "')");
					load_program_entries();
				}	
				else
					//The program has already been entered "," Entry not possible
					Windows::Forms::MessageBox::Show("Das Programm wurde schon eingetragen", "Eintrag nicht möglich");
			}
			else {
				MyInsertStatement("UPDATE Programme SET Programm='" + tB_prog->Text + "', Programm_Nr='" + programm_nr->Text + "' WHERE Gebiet_ID="+area_id+" and Programm='"+cur_prog+"'");
				load_program_entries();
			}
		}
		else
...
System::Void Datenbank::admin::btn_search_path_Click(System::Object^  sender, System::EventArgs^  e)
{
...
System::Void Datenbank::admin::btn_back_Click(System::Object^  sender, System::EventArgs^  e)
{
	if (File::Exists(tb_path->Text))
	{
		...
		array<Byte>^info = (gcnew UTF8Encoding(true))->GetBytes(tb_path->Text);
		fs->Write(info, 0, info->Length);
		fs->Close();
		//Close();
		MyRecordSet::SetPath();
